...
route.get('/', async (req, res) => {
  try {
    ...
    logger.info('GET /comments', { query: req.query });
    ...
    logger.error('Xatolik GET /comments', { error: error.message });
...
route.get('/:id', async (req, res) => {
  try {
    ...
    if (!comment) {
      logger.warn(`GET /comments/${req.params.id} - Comment not found`);
      return res.status(404).send({ message: 'comment not found' });
    }

    logger.info(`GET /comments/${req.params.id} - Success`);
    res.send(comment);
  } catch (error) {
    logger.error(`Xatolik GET /comments/${req.params.id}`, { error: error.message });
...
route.post('/', roleAuthMiddleware(['ADMIN', 'USER']), async (req, res) => {
  try {
    ...
    if (!learningExists) {
      logger.warn(`POST /comments - Learning center not found (ID: ${learningCenterId})`);
    ...
    logger.info('POST /comments - Success', { comment: newComment });
    res.send(newComment);
  } catch (error) {
    logger.error('Xatolik POST /comments', { error: error.message });
    res.status(500).send({ error: 'server error' });
  }
});
 ...
 */
route.patch('/:id', roleAuthMiddleware(['ADMIN', 'USER', 'SUPER_ADMIN']), async (req, res) => {
  try {
    const { learningCenterId, message, star } = req.body;
    const comment = await Comments.findByPk(req.params.id);

    if (!comment) {
      logger.warn(`PATCH /comments/${req.params.id} - Comment not found`);
      return res.status(404).send('Comment not found');
    }

    await comment.update({ learningCenterId, message, star });
    logger.info(`PATCH /comments/${req.params.id} - Updated successfully`);

    res.send(comment);
  } catch (error) {
    logger.error(`Xatolik PATCH /comments/${req.params.id}`, { error: error.message });
 ...
 */
route.delete('/:id', roleAuthMiddleware(['ADMIN', 'USER']), async (req, res) => {
  try {
    const comment = await Comments.findByPk(req.params.id);
    if (!comment) {
    ...
    logger.error(`Xatolik DELETE /comments/${req.params.id}`, { error: error.message });
    res.status(500).send(`Xatolik yuz berdi: ${error.message}`);
  }
});
